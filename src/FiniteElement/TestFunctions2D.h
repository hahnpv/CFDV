...
#include "FDV/Node.h"
	...
	// test function, 2d
struct testfunction : public Tensor<double, 1> // Matrix1d
{
	testfunction(Tensor<double, 1> & zeta) : Tensor<double, 1>(4) // Matrix1d(4)
	{
		(*this)(0) = 0.25 * (1-zeta(0)) * (1-zeta(1));
		(*this)(1) = 0.25 * (1+zeta(0)) * (1-zeta(1));
		(*this)(2) = 0.25 * (1+zeta(0)) * (1+zeta(1));
		(*this)(3) = 0.25 * (1-zeta(0)) * (1+zeta(1));
	}
};

	// derivative of the test function WRT natural coordinates
struct dtestfunction : public Tensor<double, 2> // Matrix2d
{
	dtestfunction(Tensor<double, 1> & zeta) : Tensor<double, 2>(4, 2) // Matrix2d(4,2)
	{
		(*this)(0, 0) = -0.25 * (1-zeta(1));		// d/xi
		(*this)(1, 0) =  0.25 * (1-zeta(1));
		(*this)(2, 0) =  0.25 * (1+zeta(1));
		(*this)(3, 0) = -0.25 * (1+zeta(1));
		(*this)(0, 1) = -0.25 * (1-zeta(0));		// d/deta
		(*this)(1, 1) = -0.25 * (1+zeta(0));
		(*this)(2, 1) =  0.25 * (1+zeta(0));
		(*this)(3, 1) =  0.25 * (1-zeta(0));
	}
};

struct d2testfunction : public Tensor<double, 1> // Matrix1d
{
	d2testfunction() : Tensor<double, 1>(4) // Matrix1d(4)
	{
		(*this)(0) =  0.25;
		(*this)(1) = -0.25;
		(*this)(2) =  0.25;
...
struct jacobian	: Tensor<double, 2> // Matrix2d		// eventaully put dx/dy dxi/deta into an array can automate more calculations, etc.
{
	jacobian(Tensor<double, 2> & dPxi, std::vector<Node *> & node) : Tensor<double, 2>(2, 2) // Matrix2d(2,2)
	{
		clear();
		for (int N = 0; N < 4; N++)
		{
...
struct dtestfunctiondx : public Tensor<double, 2> // Matrix2d
{
	dtestfunctiondx(Tensor<double, 2> & dPxi, jacobian & j) : Tensor<double, 2>(4, 2) // Matrix2d(4,2)
	{
		double dxdxi  = j(0, 0);
		double dxdeta = j(0, 1);
		double dydeta = j(1, 1);
  ...
  void tf2d(Tensor<double, 1> & Phi, double & J, Tensor<double, 2> & dPhi, Tensor<double, 1> & zeta, std::vector<Node *> & node)
  {
